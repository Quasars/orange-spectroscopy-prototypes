...
class PostProcessorHDF5Loader:
    ...
    def load_transmission(self):
        ...
        for g in f['transmission'].values():
            if 'acquisition' in g.name:
                acq_order.append(int(''.join(filter(str.isdigit, g.name)))) #this can break if acquisitions are re-named at some point in the future
        ...
        for i, v, in enumerate(acq_order):
            ...
            if self.is_timeresolved():
                tmp_transmission[v] = (f['transmission/acquisition'+str(v)]['amp'][()][:,:,0]+f['transmission/acquisition'+str(v)]['amp'][()][:,:,1]*1j)
        ...
        time_stamp_diff = [v - time_stamp[0] for v in time_stamp] # shift t = 0 to the first acquisition
        tmp_transmission = np.transpose(tmp_transmission)         
        tmp_transmission = np.array(tmp_transmission)
        ...
        if self.is_timeintegrated():
            self.data.update({'timeAxis':time_stamp_diff})
    ...
    def load_normalization(self, f, acq_order):
        ...
        if (self.config.version >= (4,1,0) and self.is_timeresolved()) or (self.config.version >= (5,0,0)):
            ...
            for i, v, in enumerate(acq_order):
                tmp_normalization[v] = f['transmission/acquisition'+str(v)]['NormalizationVector'][()][:,0,0]+f['transmission/acquisition'+str(v)]['NormalizationVector'][()][:,0,1]*1j
    ...
    def is_timeintegrated(self):
        if self.config.processor == 'TimeIntegrated' or self.config.processor == 'LongTerm' or self.config.processor == 'StepSweep' :
            return True
        else:
            return False
